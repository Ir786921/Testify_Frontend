  ...
  const features = [
    {
      ...
      description: "AI-powered matching to find the perfect candidates for your role"
    },
    {
  ...
  return (
        ...
        <div className="tw-relative tw-max-w-7xl tw-mx-auto tw-px-4 tw-py-24 sm:tw-px-6 lg:tw-px-8">
          <div className="tw-text-center tw-space-y-8">
            <h1 className="tw-text-5xl tw-font-bold tw-text-white tw-tracking-tight sm:tw-text-6xl">
              ...
              <span className="tw-block tw-text-teal-200">Smart Assessments</span>
            </h1>
            <p className="tw-max-w-2xl tw-mx-auto tw-text-xl tw-text-gray-200">
              Advanced testing platform powered by AI to evaluate and enhance your technical expertise.
            </p>
            <div className="tw-flex tw-justify-center tw-gap-4">
              <Link to={"/test"}  className=" tw-no-underline tw-inline-flex tw-items-center tw-px-8 tw-py-3 tw-rounded-full tw-text-base tw-font-medium tw-text-white tw-bg-teal-500 hover:tw-bg-teal-400 tw-transition-all tw-shadow-lg hover:tw-shadow-xl tw-border-0">
                Start Assessment
                <svg className="tw-ml-2 tw-w-5 tw-h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="tw-min-h-screen tw-bg-gradient-to-br tw-from-teal-50 tw-via-white tw-to-green-50 tw-p-5">
        ...
        {[1, 2, 3].map((step) => (
          <div
            ...
            className={`tw-bg-white tw-rounded-xl tw-p-8 tw-w-full sm:tw-w-1/3 tw-transform tw-transition-all tw-duration-300 tw-cursor-pointer
              ${activeStep === step ? 'tw-scale-105 tw-shadow-xl tw-border-2 tw-border-green-500' : 'tw-shadow-md hover:tw-shadow-lg'}
            `}
            onMouseEnter={() => setActiveStep(step)}
          >
            <div className="flex justify-center mb-6">
              {step === 1 && <i  className={`fa-solid fa-file tw-h-12 tw-w-12 tw-text-3xl ${activeStep === step ? 'tw-text-green-500' : 'tw-text-green-600'}`}></i>}
              {step === 2 && <i  className={`fa-solid fa-paper-plane tw-h-12 tw-w-12 tw-text-3xl ${activeStep === step ? 'tw-text-green-500' : 'tw-text-green-600'}`}></i>}
              {step === 3 && <i  className={`fa-solid fa-chart-bar tw-h-12 tw-w-12 tw-text-3xl ${activeStep === step ? 'tw-text-green-500' : 'tw-text-green-600'}`} ></i>}
            </div>
            <div className="tw-space-y-4">
              ...
              <p className="tw-text-gray-700 tw-text-center tw-leading-relaxed">
                {step === 1 && "Create high-quality assessments, fast. Select the tests that work for you and add custom questions."}
                {step === 2 && "Invite candidates your way. Send invites via email, ATS, or a direct link."}
                {step === 3 && "Analyze results and make decisions. Get real-time insights and compare candidates instantly."}
              </p>
            </div>
          </div>
        ))}
      ...
      <div className="tw-max-w-6xl tw-mx-auto tw-mt-20 px-4">
        <h2 className="tw-text-2xl tw-font-bold tw-text-center tw-mb-12">Why Choose Testify?</h2>
        <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-4 tw-gap-8">
          {features.map((feature, index) => (
            ...
            >
              <div className={`tw-transition-transform tw-duration-300 ${hoveredFeature === index ? 'tw-transform tw-translate-y-2' : ''}`}>
                <div className={`tw-flex items-center tw-gap-3 tw-mb-4 tw-text-green-600 ${hoveredFeature === index ? 'tw-text-green-500' : ''}`}>
                 <span> {feature.icon}</span>
                  <h3 className="tw-font-semibold tw-text-lg">{feature.title}</h3>
                </div>
      ...
      <div className="tw-py-10">
        ...
        <div className="tw-flex md:tw-flex-nowrap tw-flex-wrap tw-justify-center tw-gap-8 tw-p-5 ">
          ...
          <div className="tw-bg-white tw-rounded-lg tw-shadow-lg tw-p-6 tw-w-full sm:tw-w-1/3 tw-flex tw-flex-col tw-justify-center">
            ...
            <p className="tw-text-gray-700 tw-text-center tw-mt-2">
            Retake tests to monitor improvement and receive personalized suggestions to strengthen weak areas.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* <div className="tw-bg-gradient-to-r tw-from-green-600 tw-to-emerald-600 tw-text-white tw-mt-20 tw-py-16">
        <div className="tw-max-w-6xl tw-mx-auto tw-px-4">
          <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-12 tw-text-center">
            {[ 
              { value: "500+", label: "Companies Trust Us" },
              { value: "50,000+", label: "Assessments Created" },
              { value: "98%", label: "Satisfaction Rate" }
            ].map((stat, index) => (
              <div
                key={index}
                className="tw-transform tw-transition-all tw-duration-300 hover:tw-scale-110"
              >
                <div className="tw-text-5xl tw-font-bold tw-mb-2">{stat.value}</div>
                <div className="tw-text-lg">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="tw-flex tw-justify-center tw-p-2 tw-my-16">
        <button className="tw-group tw-relative tw-bg-green-600 tw-text-white tw-font-semibold tw-px-8 tw-py-4 tw-rounded-lg tw-shadow-md hover:bg-green-700 transition-all duration-300 hover:shadow-xl">
          <span className="tw-flex items-center tw-gap-2">
            Try Now for Free
            <i class="fa-solid fa-arrow-right tw-h-5 tw-w-5 tw-transform tw-transition-transform tw-group-hover:tw-translate-x-1"></i>
          </span>
        </button>
      </div> */}
    </div>
