...
const SystemCheck = () => {
  ...
  const  handleFullscreen = ()=>{
    ...
    if (elem.requestFullscreen) {
      elem.requestFullscreen().then(() => navigate(`/testenv/${id}`));
      ...
      navigate(`/testenv/${id}`); // Fallback if fullscreen is not supported
    }
  ...
  return (
    ...
    ${
      step === 0
        ? "tw-ring-2 tw-ring-blue-400 tw-shadow-lg tw-shadow-blue-500/20"
        : isBrowserCompatible
        ? "tw-ring-1 tw-ring-green-400"
        : "tw-ring-1 tw-ring-red-400"
    }`}
          >
        ...
        ${
          isBrowserCompatible
            ...
            : "tw-bg-red-500/20 tw-text-red-400"
              ...
              <div className="tw-bg-gradient-to-br tw-from-indigo-900/40 tw-to-purple-900/40 tw-rounded-xl tw-p-4 tw-mb-5 tw-border tw-border-white/10">
                <div className="tw-flex tw-items-center tw-p-3">
                  <div className="tw-w-12 tw-h-12 tw-bg-gradient-to-br tw-from-blue-500 tw-to-indigo-600 tw-rounded-xl tw-flex tw-items-center tw-justify-center tw-mr-4 tw-shadow-lg">
                    <span className="tw-text-white tw-text-lg tw-font-bold">
              ...
              <div
                className={`tw-rounded-xl tw-py-2 tw-px-4 tw-font-medium tw-transition-all tw-duration-300 tw-flex tw-items-center tw-justify-center
      ${
        isBrowserCompatible
          ...
          : "tw-bg-red-500/20 tw-text-red-300 tw-border tw-border-red-500/30"
              ...
              >
                ...
                <span>
                  Browser{" "}
                  {isBrowserCompatible ? "Compatible" : "Not Compatible"}
                </span>
              </div>
            </div>
          </div>
          ...
          isWebcamAvailable
            ...
            : "tw-bg-red-500/20 tw-text-red-400"
              ...
              <div className="tw-bg-gradient-to-br tw-from-indigo-900/40 tw-to-purple-900/40 tw-rounded-xl tw-p-6 tw-mb-4 tw-border tw-border-white/10 tw-flex tw-flex-col tw-items-center tw-justify-center tw-h-36">
                <div
                  className={`tw-w-16 tw-h-16 tw-rounded-full tw-bg-white/5  tw-flex tw-items-center tw-justify-center tw-mb-3 tw-border tw-border-white/10
        ${isWebcamAvailable ? "tw-shadow-lg tw-shadow-green-500/20" : ""}`}
                >
                  <i
                    className={`fa fa-camera tw-text-2xl ${
                      isWebcamAvailable ? "tw-text-blue-400" : "tw-text-white"
                    }`}
                  ></i>
                </div>
                <p
                  className={`tw-font-medium tw-text-sm ${
                    isWebcamAvailable ? "tw-text-white" : "tw-text-white"
                  }`}
                >
                  {isWebcamAvailable
        ...
        isWebcamAvailable
          ...
          : "tw-bg-red-500 tw-text-red-300 tw-border tw-border-red-500"
              ...
              >
                ...
                <span>
                  Camera{" "}
                  {isWebcamAvailable ? "Access Granted" : "Access Needed"}
                </span>
              </div>
            </div>
          </div>
            ...
            : "tw-bg-red-500/20 tw-text-red-400"
              ...
              <div className="tw-bg-gradient-to-br tw-from-indigo-900/40 tw-to-purple-900/40 tw-rounded-xl tw-p-4 tw-mb-4 tw-border tw-border-white/10">
                <div className="tw-flex tw-items-center tw-mb-3">
                  <i
                    className={`fa-solid fa-wave-square tw-h-5 tw-w-5 tw-mr-2 ${
                      isAnimating ? "tw-text-purple-400" : "tw-text-white/60"
                    }`}
                  ></i>
                  <h4 className="tw-text-white tw-font-medium">
                    Captured Audio
                  </h4>
                </div>
                <div className="tw-bg-white/5 tw-backdrop-blur-sm tw-rounded-xl tw-p-3 tw-border tw-border-white/10 tw-min-h-16 tw-flex tw-items-center">
                  ...
                  {!isAnimating && transcript ? (
                    ...
                    <p className="tw-text-white/40 tw-italic">
        ...
        ${
          isListening
            ...
            : "tw-bg-gradient-to-r tw-from-blue-500 tw-to-indigo-600 tw-text-white hover:tw-shadow-lg hover:tw-shadow-blue-500/20 tw-border tw-border-blue-400/30"
              ...
              >
                <i
                  className={`fa-solid fa-microphone tw-w-4 tw-h-4 tw-mr-2 ${
                    isListening ? "tw-animate-pulse" : ""
                  }`}
                ></i>
        ...
        ${
          isNetworkOnline
            ...
            : "tw-bg-red-500/20 tw-text-red-400"
                ...
                <div className="tw-flex tw-mb-3 tw-p-3">
                  ...
                  <div className=" tw-flex-col ">
                    ...
                    <div className="tw-flex tw-items-center tw-justify-center  tw-gap-3">
                        ...
                        {[1, 2, 3, 4, 5].map((bar) => (
                          <div
                            ...
                            className={`tw-w-1.5 tw-rounded-t-sm ${
                              bar <= Number(Math.round(Network)) / 2
                                ? "tw-bg-cyan-400"
                                : "tw-bg-white/20"
                            }`}
                      ...
                      <p className=" tw-text-sm tw-font-medium tw-text-gray-600 tw-mt-2">
                        <TestNetworkSpeed />
                      </p>
                    </div>
                  </div>
              ...
              <div
                className={`tw-rounded-xl tw-py-2 tw-px-4 tw-font-medium tw-transition-all tw-duration-300 tw-flex tw-items-center tw-justify-center tw-gap-5
      ${
        isNetworkOnline
          ...
          {errorMsg.length === 0 ? (
            <div className="tw-bg-emerald-50 tw-border tw-border-emerald-200 tw-rounded-lg tw-overflow-hidden tw-shadow-sm">
              <div className="tw-px-4 tw-py-3 tw-flex tw-items-center tw-gap-3">
                <span className="tw-flex-shrink-0 tw-w-8 tw-h-8 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-bg-emerald-100">
                  <svg
                    className="tw-w-5 tw-h-5 tw-text-emerald-600"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                {step === 3 ? (
                  <p className="tw-text-emerald-700 tw-font-medium">
                    <span className="tw-text-emerald-600">
                      Great! All hardware checks passed. You're ready to
                      proceed.
                    </span>
                  </p>
                ) : (
                  <p className="text-yellow-700 font-medium">
                    <span className="font-bold">Important:</span> It is
                    mandatory to check your microphone first before proceeding.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="tw-space-y-3">
              {errorMsg.map((msg, index) => (
                <div
                  key={index}
                  className="tw-bg-red-50 tw-border tw-border-red-200 tw-rounded-lg tw-overflow-hidden tw-shadow-sm"
                >
                  <div className="tw-px-4 tw-py-3 tw-flex tw-items-center tw-gap-3">
